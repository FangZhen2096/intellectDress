...
- (instancetype)init {
    CGRect frame = CGRectMake(4, 0, DEVICE_WIDTH-8, 46);
...
- (void)setupItems {
    ...
    for (NSString *title in _itemsTitle) {
        ...
        item.delegate = self;
        [item setItemTitleColor:[[UIColor redColor] colorWithAlphaComponent:0.5]];
...
- (void)selectSlideBarItemAtIndex:(NSUInteger)index {
    ...
    [self refreshContenOffset:item];
}

#pragma mark - FDSlideBarItemDelegate

//选中的居中
- (void)refreshContenOffset:(FDSlideBarItem *)item {
    //    if (item.centerX>=_scrollView.centerX) {
    //        CGFloat delta=   item.centerX-_scrollView.centerX;
    ////        _scrollView.contentOffset = CGPointMake(delta, 0);
    ////        if (delta<=_scrollView.contentOffset.x) {
    //        if (item.x+_scrollView.centerX>_scrollView.contentSize.width) {
    //            [_scrollView setContentOffset:CGPointMake(_scrollView.contentSize.width-self.width, 0) animated:YES];
    //        }else{
    //                    [_scrollView setContentOffset:CGPointMake(delta, 0) animated:YES];
    //        }
    //
    ////        }
    //
    ...
    //如果item.x 宽度大于scrollView宽度的一半 scrollView 移动使item居中
    if (itemX > width/2) {
        CGFloat targetX;
